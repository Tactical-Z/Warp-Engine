...
vec2* Readhdf5File(const char* _file, const char* _datasetX, const char* _datasetY){
    ...
    vec2* vecField = malloc(sizeof(vec2) * totalSize);
    if (!vecField) {
        free(xData);
        free(yData);
    ...
    for (size_t i = 0; i < totalSize; i++) {
        vecField[i][0] = xData[i];
        vecField[i][1] = yData[i];
    }

    //LOG_DEBUG("First vector: (%f, %f)", vecField[0][0], vecField[0][1]);
    PrintVectorField(vecField, &metaData);
